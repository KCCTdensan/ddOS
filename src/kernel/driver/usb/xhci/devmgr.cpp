...
namespace usb::xhci {
  kError DeviceManager::Initialize(size_t max_slots) {
    max_slots_ = max_slots;

    devices_ = AllocArray<Device*>(max_slots_ + 1, 0, 0);
    if (devices_ == nullptr) {
      return KernelError(kError::kNoEnoughMemory);
    }
  ...
  /*
  WithkError<Device*> DeviceManager::Get(uint8_t device_id) const {
    if (device_id >= num_devices_) {
      return {nullptr, kError::kInvalidDeviceId};
    }
    return {&devices_[device_id], kError::kSuccess};
  }
  */

  kError DeviceManager::AllocDevice(uint8_t slot_id, DoorbellRegister* dbreg) {
    if (slot_id > max_slots_) {
      return KernelError(kError::kInvalidSlotID);
    }

    if (devices_[slot_id] != nullptr) {
      return KernelError(kError::kAlreadyAllocated);
    }
  ...
  kError DeviceManager::LoadDCBAA(uint8_t slot_id) {
    if (slot_id > max_slots_) {
      return KernelError(kError::kInvalidSlotID);
    }
  ...
  kError DeviceManager::Remove(uint8_t slot_id) {
